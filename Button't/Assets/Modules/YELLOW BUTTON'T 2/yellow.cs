...
public class yellow : MonoBehaviour
{
    ...
    string[] names = {"ABACUS", "ABDUCT", "ABJECT", "ABLAZE", "ABOARD", "ABOUND", "ABROAD", "ABRUPT", "ABSENT", "ABSORB", "ABSURD", "ABUSED", "ABUSER", "ABUSES", "ACACIA", "ACCEDE", "ACCENT", "ACCEPT", "ACCESS", "ACCORD", "ACCOST", "ACCRUE", "ACCUSE", "ACETIC", "ACHING", "ACIDIC", "ACQUIT", "ACROSS", "ACTING", "ACTION", "ACTIVE", "ACTORS", "ACTUAL", "ACUITY", "ACUMEN", "ADAGIO", "ADDICT", "ADDING", "ADHERE", "ADJOIN", "ADJUST", "ADMIRE", "ADMITS", "ADORED", "ADRIFT", "ADULTS", "ADVENT", "ADVERB", "ADVERT", "ADVICE", "ADVISE", "AERIAL", "AFFAIR", "AFFECT", "AFFINE", "AFFIRM", "AFFORD", "AFGHAN", "AFIELD", "AFLAME", "AFLOAT", "AFRAID", "AFRESH", "AFRICA", "AGENCY", "AGENDA", "AGENTS", "AGHAST", "AGREED", "AGREES", "AIMING", "AIRBAG", "AIRING", "ALASKA", "ALBEIT", "ALBINO", "ALBUMS", "ALCOVE", "ALIENS", "ALKALI", "ALLEGE", "ALLELE", "ALLIED", "ALLIES", "ALLOWS", "ALLUDE", "ALLURE", "ALMOND", "ALMOST", "ALPACA", "ALPINE", "ALUMNA", "ALUMNI", "ALWAYS", "AMAZED", "AMAZON", "AMBUSH", "AMENDS", "AMIDST", "AMOEBA", "AMORAL", "AMOUNT", "AMULET", "AMUSED", "ANALOG", "ANCHOR", "ANEMIA", "ANEMIC", "ANGELS", "ANGINA", "ANGLED", "ANGLER", "ANGLES", "ANGORA", "ANIMAL", "ANKLES", "ANKLET", "ANNUAL", "ANOINT", "ANORAK", "ANSWER", "ANTHEM", "ANTICS", "ANTLER", "ANYHOW", "ANYONE", "ANYWAY", "AORTIC", "APACHE", "APATHY", "APIECE", "APOGEE", "APPALL", "APPEAL", "APPEAR", "APPEND", "APPLES", "APPLET", "ARCADE", "ARCANA", "ARCANE", "ARCHED", "ARCHER", "ARCHES", "ARCHLY", "ARCTIC", "ARDENT", "ARGUED", "ARGUES", "ARISEN", "ARISES", "ARMADA", "ARMFUL", "ARMIES", "ARMING", "ARMORY", "ARMOUR", "ARMPIT", "AROUND", "AROUSE", "ARREST", "ARRIVE", "ARROWS", "ARTERY", "ARTFUL", "ARTIST", "ASCEND", "ASCENT", "ASHORE", "ASKING", "ASLEEP", "ASPECT", "ASPIRE", "ASSENT", "ASSERT", "ASSESS", "ASSETS", "ASSIGN", "ASSIST", "ASSUME", "ASSURE", "ASTHMA", "ASTRAL", "ASTRAY", "ASTUTE", "ASYLUM", "ATOMIC", "ATONAL", "ATRIUM", "ATTACH", "ATTACK", "ATTAIN", "ATTEND", "ATTEST", "ATTIRE", "ATTUNE", "AUGUST", "AUNTIE", "AUTHOR", "AUTISM", "AUTUMN", "AVATAR", "AVENGE", "AVENUE", "AVERSE", "AVIARY", "AVIDLY", "AVOIDS", "AVOWAL", "AVOWED", "AWAKEN", "AWARDS", "AWHILE", "AWNING", "AZALEA",
                ...
                "AXO","RUBIX","HI","NEWIMAGE"
    };

    string[] FORMATES = { "IMAGE","VIDEO","MUSIC","TEXT","APP","OTHER"};

    string[] image = {"JPG","JPEG","SVG","PNG"};
    string[] video = {"MP4","AVI","MKV","WMV"};
    string[] music = {"MP3","WAV","OGG","WMA"};
    string[] text = {"CS","TXT","JSON","CSV","DOC","DOCX"};
    string[] app = {"EXE","PY"};
    string[] other = {"ISO","XYZ","RET","MAE"};
    ...
    private void Start()
    {
        ...
        size = Rnd.Range(1, 1024);
        nameq = names[Rnd.Range(0, names.Length)];
        formates = FORMATES[Rnd.Range(0, FORMATES.Length)];
        stage = 0;
        if (formates == "IMAGE")
        {
            aa = image[Rnd.Range(0, image.Length)];
        }
        else if (formates == "VIDEO") 
        {
            aa = video[Rnd.Range(0, video.Length)];
        }
        else if (formates == "MUSIC") 
        {
            aa = music[Rnd.Range(0, music.Length)];
        }
        else if (formates == "TEXT") 
        {
            aa = text[Rnd.Range(0, text.Length)];
        }
        else if (formates == "APP") 
        {
            aa = app[Rnd.Range(0, app.Length)];
        }
        else if (formates == "OTHER") 
        {
            aa = other[Rnd.Range(0, other.Length)];
        }
        DisplayText.text = string.Concat(nameq,".",aa);
        ButtonText.text = string.Concat(size.ToString(), "MB");

        if(size > 50 & formates == "IMAGE")
        {
            b1 = 1;
        }
        else
        ...
        if (b1 == 1 | b2 == 1 | b3 == 1 | b4 == 1 | b5 == 1 | b6 == 1 | b8 == 1)
        {
            b7 = 1;
        }
        else
        ...
        Debug.LogFormat("[Yellow Button't #{0}] Binary:{1}", _moduleId,string.Concat(b1.ToString(), b2.ToString(), b3.ToString(), b4.ToString(), b5.ToString(), b6.ToString(), b7.ToString(), b8.ToString()));

        if (b1 == 1)
        {
            Debug.LogFormat("[Yellow Button't #{0}] Tap the button when the last two digits of the timer is an even number {1}", _moduleId, ".");
        }
        else
        {
            Debug.LogFormat("[Yellow Button't #{0}] Tap the button when the last two digits of the timer is an odd number {1}", _moduleId,".");
        }
        if (b2 == 1)
        {
            Debug.LogFormat("[Yellow Button't #{0}] Tap the button when the last two digits of the timer is an odd number {1}", _moduleId, ".");
        ...
        if (b5 == 1)
        {
            Debug.LogFormat("[Yellow Button't #{0}] Tap the button when the last two digits of the timer is 0,3,6,9,12,15,18,21,24,27,ETC... {1}", _moduleId, ".");
        }
        else
        {
            Debug.LogFormat("[Yellow Button't #{0}] Tap the button when the last two digits of the timer is NOT 0,3,6,9,12,15,18,21,24,27,ETC... {1}", _moduleId, ".");
        }
        if (b6 == 1)
        {
            Debug.LogFormat("[Yellow Button't #{0}] Tap the button when the last two digits of the timer is  {1}", _moduleId, size.ToString());
        }
        else
        {
            Debug.LogFormat("[Yellow Button't #{0}] Tap the button when the last two digits of the timer is NOT {1}", _moduleId, (size%60).ToString());
        }
        if (b7 == 1)
        {
            Debug.LogFormat("[Yellow Button't #{0}] Tap the button when the last two digits of the timer is an even number {1}", _moduleId, ".");
        ...
        if (b8 == 1)
        {
            Debug.LogFormat("[Yellow Button't #{0}] Tap the button when the last two digits of the timer is a number > 50 {1}", _moduleId, ".");
        }
        else
        {
            Debug.LogFormat("[Yellow Button't #{0}] Tap the button when the last two digits of the timer is a number =< 50 {1}", _moduleId, ".");
    ...
    private bool ButtonPress()
    {
        ...
        if (!_moduleSolved)
        {
            ...
            else if (stage == 5)
            {
                if (b6 == 6)
                {
                    if ((int)BombInfo.GetTime()  == size % 60)
                    {
                        stage += 1;
                    }
                    else
...
#pragma warning restore 0414
    ...
    public IEnumerator TwitchHandleForcedSolve()
    {
        ...
        if (b5 == 1)
        {
            while ((int)BombInfo.GetTime() % 3 != 0)
                yield return "trycancel";
            ButtonSelectable.OnInteract();
        }
        else
        {
            while ((int)BombInfo.GetTime() % 3 != 1)
                yield return "trycancel";
            ButtonSelectable.OnInteract();
        }
        if (b6 == 1)
        {
            while ((int)BombInfo.GetTime() % 60 != size%60)
                yield return "trycancel";
            ButtonSelectable.OnInteract();
        }
        else
        ...
        if (b8 == 1)
        {
            while ((int)BombInfo.GetTime() % 60 > 50)
                yield return "trycancel";
            ButtonSelectable.OnInteract();
        }
        else
