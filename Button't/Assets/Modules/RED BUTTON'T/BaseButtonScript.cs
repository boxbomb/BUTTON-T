...
public class BaseButtonScript : MonoBehaviour
{
    ...
    private static readonly string[] flash = { "SUBMIT", "LEFT", "UP", "RIGHT", "DOWN", "RESET" };
    public string selectkeyword;
    public string resultkeyword;
    public string alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXY";
    ...
    private void Start()
    {
        ...
        alphabet = string.Concat(alphabet, n);
        ...
        alphabet = alphabet.Remove(0, 1);
        filltoletter = alphabet.Substring(0, 1);
        Tableletter[3, 4] = filltoletter;
        alphabet = alphabet.Remove(0, 1);
        filltoletter = alphabet.Substring(0, 1);
        Tableletter[4, 0] = filltoletter;
        alphabet = alphabet.Remove(0, 1);
        filltoletter = alphabet.Substring(0, 1);
        Tableletter[4, 1] = filltoletter;
        alphabet = alphabet.Remove(0, 1);
        filltoletter = alphabet.Substring(0, 1);
        Tableletter[4, 2] = filltoletter;
        alphabet = alphabet.Remove(0, 1);
        filltoletter = alphabet.Substring(0, 1);
        Tableletter[4, 3] = filltoletter;
        alphabet = alphabet.Remove(0, 1);
        filltoletter = alphabet.Substring(0, 1);
        Tableletter[4, 4] = filltoletter;
        alphabet = alphabet.Remove(0, 1);
        startX = Rnd.Range(0, 5);
        startY = Rnd.Range(0, 5);
        endX = Rnd.Range(0, 5);
        endY = Rnd.Range(0, 5);
        curX = startX;
        curY = startY;
        ...
        if (startX - endX > 0) 
        {
            routenum = Math.Abs(startX - endX);
            print(routenum.ToString());
            while(routenum > 0)
            {
                route = string.Concat(route, "u");
                routenum -= 1;
            }
        }
        ...
        if (startY - endY > 0)
        {
            routenum = Math.Abs(startY - endY);
            print(routenum.ToString());
            while (routenum > 0)
            {
                route = string.Concat(route, "l");
                routenum -= 1;
            }
        }
        ...
        else if (startY - endY < 0)
        {
            ...
            while (routenum > 0)
            {
                route = string.Concat(route, "r");
                routenum -= 1;
            }
        }
        ButtonText.text = string.Concat(Tableletter[startX, startY], Tableletter[endX, endY]);

        Debug.LogFormat("[Red Button't #{0}] Shortest route: {1}", _moduleId, route);
        Debug.LogFormat("[Red Button't #{0}] Start Location: {1}", _moduleId, Tableletter[startX, startY]);
        Debug.LogFormat("[Red Button't #{0}] End Location: {1}", _moduleId, Tableletter[endX, endY]);

        Debug.LogFormat("[Red Button't #{0}] 1 {1}", _moduleId, string.Concat(Tableletter[0, 0], Tableletter[0, 1], Tableletter[0, 2], Tableletter[0, 3], Tableletter[0, 4]));
        Debug.LogFormat("[Red Button't #{0}] 2 {1}", _moduleId, string.Concat(Tableletter[1, 0], Tableletter[1, 1], Tableletter[1, 2], Tableletter[1, 3], Tableletter[1, 4]));
        Debug.LogFormat("[Red Button't #{0}] 3 {1}", _moduleId, string.Concat(Tableletter[2, 0], Tableletter[2, 1], Tableletter[2, 2], Tableletter[2, 3], Tableletter[2, 4]));
        Debug.LogFormat("[Red Button't #{0}] 4 {1}", _moduleId, string.Concat(Tableletter[3, 0], Tableletter[3, 1], Tableletter[3, 2], Tableletter[3, 3], Tableletter[3, 4]));
        Debug.LogFormat("[Red Button't #{0}] 5 {1}", _moduleId, string.Concat(Tableletter[4, 0], Tableletter[4, 1], Tableletter[4, 2], Tableletter[4, 3], Tableletter[4, 4]));
    ...
    private bool ButtonPress()
    {
        ...
        if (!_moduleSolved)
        {
            Debug.LogFormat("[Red Button't #{0}] Held the button", _moduleId);
        }
        return false;
    }
    ...
    private void ButtonRelease()
    {
        ...
        if (!_moduleSolved)
        {
            if (DisplayText.text == "LEFT")
            {
                curY -= 1;
                Debug.LogFormat("[Red Button't #{0}] Release at LEFT", _moduleId);
            }
            else if (DisplayText.text == "UP")
            {
                curX -= 1;
                Debug.LogFormat("[Red Button't #{0}] Release at UP ", _moduleId);
            }
            else if (DisplayText.text == "RIGHT")
            {
                curY += 1;
                Debug.LogFormat("[Red Button't #{0}] Release at RIGHT ", _moduleId);
            }
            else if (DisplayText.text == "DOWN")
            {
                curX += 1;
                Debug.LogFormat("[Red Button't #{0}] Release at DOWN ", _moduleId);
            }
            else if (DisplayText.text == "RESET")
            {
                curX = startX;
                curY = startY;
                Debug.LogFormat("[Red Button't #{0}] Release at RESET", _moduleId);
            }
            else if (DisplayText.text == "SUBMIT")
            {
                if (Math.Abs(curX % 5 )== endX & Math.Abs(curY % 5) == endY)
                {
                    
                    GetComponent<KMBombModule>().HandlePass();
                    Debug.LogFormat("[Red Button't #{0}] SOLVED!!!", _moduleId);
                    _moduleSolved = true;
                }
                else
                {
                    GetComponent<KMBombModule>().HandleStrike();
                    Debug.LogFormat("[Red Button't #{0}] STRIKE!!!", _moduleId);
                }
    ...
    private IEnumerator TextFlash()
    {
        while (!_moduleSolved)
        {
            ...
            {    
                ...
                texttime += 1;
                yield return new WaitForSeconds(.5f);
                DisplayText.text = keyword[tableN];
                texttime = 0; 
                yield return new WaitForSeconds(.5f); 
...
#pragma warning disable 0414
    public readonly string TwitchHelpMessage = @"!{0} LEFT to hold and release at LEFT";
